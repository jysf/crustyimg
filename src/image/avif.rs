...
mod tests {
    ...
    ///
    /// SPEC-107 AC-4: reads the COMMITTED `tests/fixtures/hostile/empty_alpha_obu.avif`
    /// (generated by [`generate_empty_alpha_obu_fixture`] below) rather than
    /// rebuilding the bytes in-process, so the artifact the native CLI and
    /// wasm harnesses drive is provably the same one this guard is pinned
    /// against — two copies of these bytes that could drift apart is the
    /// failure mode SPEC-107 exists to avoid.
    #[test]
    fn empty_alpha_obu_is_typed_error_not_abort() {
        const BYTES: &[u8] = include_bytes!("../../tests/fixtures/hostile/empty_alpha_obu.avif");
    ...
    /// Regenerates the committed `tests/fixtures/hostile/empty_alpha_obu.avif`
    /// (SPEC-107, AC-4) — the exact bytes [`build_avif_with_empty_alpha`]
    /// produces over the committed 16x16 fixture's real primary OBU. Not part
    /// of the normal suite; run manually after touching the builder above:
    /// `cargo test --lib generate_empty_alpha_obu_fixture -- --ignored`. The
    /// checked-in file is expected to be byte-identical to this output — it is
    /// the reviewable source, not an opaque blob.
    #[test]
    #[ignore = "regenerates a committed fixture; run manually, not part of the suite"]
    fn generate_empty_alpha_obu_fixture() {
        let bytes = build_avif_with_empty_alpha(&real_primary_obu());
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/hostile/empty_alpha_obu.avif");
