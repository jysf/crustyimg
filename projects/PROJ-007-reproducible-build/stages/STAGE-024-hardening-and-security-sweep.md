...
- [ ] (not yet framed, **NEXT — user-prioritized, from SPEC-069 verify**) — **SPEC-070: bound peak decode memory (Medium).** `image::Limits.max_alloc` bounds a single allocation, not the cumulative/peak working set → a crafted near-max-dimension image (AVIF iloc / JPEG SOF) peaks ~1.9 GB while passing every DEC-034 cap (F-RAW-1 + F-AVIF-3's shared root; product-facing — `info` on a 782 B `.nef` → ~1.93 GB). Fix = a total-pixel / peak-bytes budget enforced via a pre-decode dimension peek at each decode entry (AVIF `check_caps` + SVG tree-size already have dims; RAW-preview + general JPEG/PNG need a header peek), + a DEC for the budget/amplification-factor tradeoff (reject the 160 MP bomb, keep legitimate ~24 MP photos). No new dep.
...
**Count:** 2 shipped / 0 active / 9 pending — SPEC-068 (LEAD security review) + SPEC-069 (decoder fuzz gate) shipped. **Next to frame: SPEC-070 (bound peak decode memory, Medium) — user-prioritized ahead of the rest.** 4 items carried from SPEC-067 (×2) + SPEC-068 (×1) + SPEC-069 (×1) verify.
