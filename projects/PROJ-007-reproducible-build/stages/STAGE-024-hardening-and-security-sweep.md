...
- [ ] (not yet framed, from SPEC-070 due-diligence — **NEXT, small**) — **lint `LimitsExceeded` false-diagnosis:** `src/lint/mod.rs` `TruncatedOrCorrupt` rule's `Err(_)` catch-all calls a valid-but-too-large image "truncated or corrupt … re-export a valid image" (destructive false advice). Pre-existing (over-dimension images already hit it), but SPEC-070's 64 Mpix cap widens its reach. Fix = add a `LimitsExceeded => None` (or a distinct honest finding) arm, mirroring the existing `CodecNotBuilt` special-case (SPEC-062). The [IMAGE_EXTENSIONS-exposes-every-decode-caller] lesson, 4th time. Small + a test.
- [ ] (not yet framed, from SPEC-070 verify) — **full-pipeline peak envelope:** DEC-063's 1 GiB budget governs DECODE only; encode/rule buffers stack on top (a legit 49 Mpix `convert` hit 934 MB, at-cap GIF `lint` 776 MB). Document/bound the true decode+pipeline peak, or state the envelope where the cap is documented. Med.
- [ ] (not yet framed, from SPEC-070 cap decision) — **`--max-pixels` opt-in / raise the 64 Mpix cap:** DEC-063 keeps 64 Mpix now; the maintainer flagged likely future medium-format interest (revisit trigger is live). The honest fix is a documented opt-in flag/config, not a silently higher default — its own spec (an escape hatch to a security bound is a decision).
...
**Count:** 3 shipped / 0 active / 10 pending — SPEC-068 + SPEC-069 + SPEC-070 shipped. **Next to frame: the lint `LimitsExceeded` false-diagnosis (small).** 7 items carried from SPEC-067 (×2) + SPEC-068 (×1) + SPEC-069 (×1) + SPEC-070 (×3) verify/due-diligence.
