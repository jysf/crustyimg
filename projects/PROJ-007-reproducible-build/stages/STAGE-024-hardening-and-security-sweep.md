...
- [~] SPEC-071 (design) — **STAGE-024 hardening cleanup (batch, Medium).** Framed + build-ready 2026-07-11. Four small, independent point-fixes in ONE cycle (the tail is disproportionate to a full spec each): (1) **lint `LimitsExceeded` false-diagnosis** — `src/lint/mod.rs:576` `TruncatedOrCorrupt` `Err(_)` catch-all calls a valid-but-too-large image "truncated or corrupt / re-export"; add a `LimitsExceeded => None` arm mirroring the `CodecNotBuilt` special-case (SPEC-062); the [IMAGE_EXTENSIONS-exposes-every-decode-caller] lesson, 4th time; (2) **cache off-by-53** — `store_bounded` bounds the payload while `read_entry` bounds the frame (payload+53+ext); bound the frame consistently + rework the `:855` documenting test; (3) **exit-code `is_total` value-assertions** — `code()` is already compiler-exhaustive → add `Metadata::Container`/`Exif` (+ confirm `Watch::Watch`) value assertions (test-completeness); (4) **`--watch` build-only** — reject on non-build subcommands (exit 2, decided: reject); (5) **docs sync** (added pre-ship) — `api-contract.md` decode-limits missing SPEC-070's 64 Mpix cap (DEC-063) + stale `--max-pixels` "planned" line; `cli-reference.md` has no `--watch` doc → document it + build-only. No new dep, no new DEC. Drive the binary for the lint + `--watch` strings.
...
**Count:** 3 shipped / 1 active / 7 pending — SPEC-068 + SPEC-069 + SPEC-070 shipped; **SPEC-071 (hardening cleanup batch) framed + build-ready**, folding 4 small items. Remaining 7 are the larger/decision-bearing tail (format-sniff, canonicalize-out, `--max-pixels`, cache-key profile, unusual-filename sweep, full-pipeline envelope, orphan prune) — after SPEC-071, re-assess which to build vs defer before closing PROJ-007.
