...
- [~] **build** — Sonnet 4.6, fresh session, per `prompts/SPEC-016-build.md`: new `src/quality/` module (SSIMULACRA2 metric + generic quality search) + `ssimulacra2` default dep (DEC-018 `just deny`) + `shrink --target`/`--ssim` wiring. Branch `feat/spec-016-perceptual-auto-quality-shrink-to-a-visual-target`.
