...
- [ ] **build** — Sonnet 4.6 (or orchestrator-direct if subagent Bash blocked), per `prompts/SPEC-018-build.md`: `avif = ["image/avif"]` feature + scoped deny exception + sink AVIF encode (`CodecNotBuilt`→exit 4 without the feature) + quality AVIF arm + CI `--features avif` job. Branch `feat/spec-018-avif-output-behind-a-feature-gated-ravif-codec`.
