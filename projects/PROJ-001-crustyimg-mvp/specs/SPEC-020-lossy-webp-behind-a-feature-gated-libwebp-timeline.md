...
- [ ] **build** — Sonnet 4.6 (or orchestrator-direct if subagent Bash blocked), per `prompts/SPEC-020-build.md`: `webp` optional dep + `webp-lossy = ["dep:webp"]`; sink WebP lossy arm (lossy iff quality set, else lossless fall-through); quality WebP arm + BOTH predicates; `--features webp-lossy` CI job; docs. NO default-build change, NO new exception, NO webp `image` feature. Branch `feat/spec-020-lossy-webp-behind-a-feature-gated-libwebp`.
