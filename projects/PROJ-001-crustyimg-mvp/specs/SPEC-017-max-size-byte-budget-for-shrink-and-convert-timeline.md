...
- [ ] **build** — Sonnet 4.6 (or orchestrator-direct if subagent Bash is blocked), per `prompts/SPEC-017-build.md`: `search_threshold` refactor + `search_jpeg_under_size`/`auto_jpeg_under_size` + `parse_size` + `AutoQuality` + `--max-size` on shrink/convert. Branch `feat/spec-017-max-size-byte-budget-for-shrink-and-convert`.
