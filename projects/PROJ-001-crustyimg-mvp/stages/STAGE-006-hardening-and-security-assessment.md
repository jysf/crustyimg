...
- [ ] SPEC-036 (design 2026-06-19) — extend the CI cargo-deny gate from `check licenses` to the full `check advisories bans sources licenses` (+ `deny.toml` sections + `just deny`); `cargo audit` consolidated into cargo-deny's RUSTSEC advisories check (DEC-037), not a separate job; no new runtime dep
...
**Count:** 3 shipped / 1 in design / 1 pending
