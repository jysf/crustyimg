...
- [~] SPEC-042 (design 2026-07-03) — release channels #4 + #5: Homebrew tap via cargo-dist (`installers += homebrew`, `tap = jysf/homebrew-tap`, `publish-jobs = [homebrew]` → a `publish-homebrew-formula` job using `HOMEBREW_TAP_TOKEN`) + a SEPARATE tag-only `publish-crates.yml` running `cargo publish --locked` with `CARGO_REGISTRY_TOKEN` (cargo-dist can't publish to crates.io — DEC-041). DEC-041. **Config only — arms both channels on the next `v*` tag; creating the tap repo, adding the 2 secrets, and cutting the tag are maintainer-authorized (not in the spec).**
...
**Count:** 4 shipped / 1 active / 2 pending  (#1 publish-ready crate, #2 changelog/release docs, #6 README/completions [SPEC-040], #3 release pipeline + MSRV [SPEC-041] shipped. #4 Homebrew tap + #5 crates.io publish [SPEC-042, design done] active — config that ARMS both channels on the next `v*` tag; the tap repo + secrets + tag cut are maintainer-authorized, not in the spec. Pending: #7 dual lean/full artifacts (DEFERRED to a fast-follow — not a native cargo-dist artifact, DEC-041). The first real `v0.1.0` tag/release/publish cut is itself maintainer-authorized — not a spec.)
