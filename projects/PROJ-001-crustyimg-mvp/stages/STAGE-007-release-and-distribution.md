...
**Count:** 4 shipped / 0 active / 3 pending  (#1 publish-ready crate, #2 changelog/release docs, #6 README/completions [SPEC-040], #3 release pipeline + MSRV [SPEC-041] shipped — the pipeline is ARMED. Remaining pending are all OUTWARD-FACING / [MAINTAINER-AUTHORIZED] and extend the SPEC-041 cargo-dist config: #4 Homebrew tap (add `homebrew` installer + create `jysf/homebrew-tap`), #5 `cargo publish` (add crates.io `publish-jobs` + token), #7 dual lean/full artifacts. The first real `v0.1.0` tag/release cut is itself maintainer-authorized — not a spec.)
