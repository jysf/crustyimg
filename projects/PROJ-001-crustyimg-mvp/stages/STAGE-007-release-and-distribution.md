...
- [x] SPEC-041 (shipped 2026-06-23, PR #45) — release CI pipeline (cargo-dist `dist` 0.32.0): `dist-workspace.toml` (4 targets: macOS arm64/x86_64, Linux x86_64, Windows; shell+powershell installers) + generated tag-triggered `release.yml` (PR=non-publishing plan; `v*` tag=publish; GitHub-Releases-only, no crates.io/tap) + `[profile.dist]` + declared MSRV `rust-version = "1.89.0"` (true dep floor; CI-enforced by an `msrv` job). DEC-040. **Pipeline armed; no release cut** — `dist plan` dry-run + the PR's non-publishing `plan` job verified it; the actual `v0.1.0` tag/release remains maintainer-authorized.
- [x] SPEC-042 (shipped 2026-07-03, PR #46) — release channels #4 + #5: Homebrew tap via cargo-dist (`installers += homebrew`, `tap = jysf/homebrew-tap`, `publish-jobs = [homebrew]` → a `publish-homebrew-formula` job using `HOMEBREW_TAP_TOKEN`) + a SEPARATE tag-only `publish-crates.yml` running `cargo publish --locked` with `CARGO_REGISTRY_TOKEN` (cargo-dist can't publish to crates.io — DEC-041). DEC-041. **Channels ARMED** — the `jysf/homebrew-tap` repo + both secrets are now in place; the `v0.1.0` tag is the remaining maintainer-authorized trigger.
...
- [x] SPEC-043 (shipped 2026-07-03, PR #47) — **unplanned supply-chain repair** (not a #1–#7 item): greened the `cargo deny` advisories gate that had gone red on `main` from ambient RustSec DB drift — 3 documented, revisit-tracked `deny.toml` ignores (quick-xml RUSTSEC-2026-0194/-0195 via little_exif; ttf-parser -0192 via ab_glyph) backed by a reachability assessment (DEC-042). No code/dep change. Unblocked SPEC-042 + the release.

**Count:** 6 shipped / 0 active / 1 pending  (#1 publish-ready crate, #2 changelog/release docs, #3 release pipeline + MSRV [SPEC-041], #4 Homebrew tap + #5 crates.io publish [SPEC-042], #6 README/completions [SPEC-040] shipped — plus the unplanned SPEC-043 supply-chain repair. **All release machinery is ARMED**: pipeline + Homebrew tap + crates.io, the `jysf/homebrew-tap` repo, and both secrets are in place. Pending: **#7 dual lean/full artifacts** (DEFERRED to a fast-follow — not a native cargo-dist artifact, DEC-041). The only thing left to actually launch is the maintainer-authorized **`v0.1.0` tag cut** (RELEASING.md) — not a spec — **and #5's actual crates.io publish is now additionally blocked on the DEC-078 caret migration of the library-public deps**, which must land before that tag is pushed.)
