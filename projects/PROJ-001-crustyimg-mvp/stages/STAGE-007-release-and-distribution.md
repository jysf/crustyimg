...
**Count:** 6 shipped / 0 active / 1 pending  (#1 publish-ready crate, #2 changelog/release docs, #3 release pipeline + MSRV [SPEC-041], #4 Homebrew tap + #5 crates.io publish [SPEC-042], #6 README/completions [SPEC-040] shipped — plus the unplanned SPEC-043 supply-chain repair. **All release machinery has FIRED**: the pipeline, Homebrew tap, and crates.io publish have run on every tag since `v0.1.0` (2026-07-04); crustyimg is on crates.io now (latest 0.4.0, `has_lib: true`). Pending: **#7 dual lean/full artifacts** (DEFERRED to a fast-follow — not a native cargo-dist artifact, DEC-041). The library-public dependency caret migration this stage's release machinery depended on is DEC-079 (SPEC-099), superseding the now-corrected DEC-078.)
