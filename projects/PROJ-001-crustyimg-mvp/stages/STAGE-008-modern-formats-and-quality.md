...
- [~] SPEC-019 (design) — **WebP lossless output + WebP decode (input), pure-Rust DEFAULT:** add `webp` to the image default features so `.webp` reads as INPUT everywhere and `convert --format webp` / `-o x.webp` write LOSSLESS WebP (smaller than PNG). All pure-Rust, no system deps, `just deny` green with NO new exception; **DEC-021**. No quality knob → `-q`/auto ignored like PNG (not a `LossyFormat`). The WebP foundation; lossy is SPEC-020. (Empirically verified: image-webp decodes lossy+lossless but encodes lossless only.)
- [ ] (not yet written) SPEC-020 — **Lossy WebP encode (feature-gated `webp-lossy`):** wire `libwebp` (a C system dep — the project's first) behind an off-by-default `webp-lossy` feature so `convert --format webp -q Q` and the auto-quality searches (`--target`/`--ssim`/`--max-size`) drive WebP (BOTH searches — the pure-Rust decoder already exists, so perceptual can score WebP round-trips). Needs its own DEC (DEC-022: the C-dep decision, CI C-toolchain, `single-image-library` tension) + dep verification. The smaller-than-JPEG story. Layers onto SPEC-019's WebP wiring

**Count:** 3 shipped / 1 in design (SPEC-019) / 2 pending (dimension-reduction fallback, SPEC-020 lossy WebP)
