...
cost:
  sessions:
    ...
    - cycle: build
      ...
      interface: claude-code
      tokens_total: 84784512
      duration_minutes: 60
      recorded_at: 2026-08-18
      tokens_breakdown:
        input: 564
        output: 185851
        cache_creation: 420934
        cache_read: 84177163
      estimated_usd: 29.62
      note: >
        ...
        the group, output taken as MAX. Was $58.50 / 165,160,452 (1.98x over) across
        the same 555 transcript lines = 282 real API calls. See STAGE-053.
    - cycle: build
      agent: claude-opus-5
      interface: claude-code
      ...
      note: >
        ...
        **9.5 %**. 172 messages against the prompt's ~150 budget; the matrix
        ...
        input/cache_creation/cache_read, so those three were double-counted once per
        extra block. Recomputed by deduping on `.message.id` — those three taken from
        the group, output taken as MAX. Was $21.59 / 28,806,619 (1.77x over) across
        the same 172 transcript lines = 99 real API calls. See STAGE-053.
    - cycle: verify
      agent: claude-opus-5
      interface: claude-code
      ...
      note: >
        ...
        input/cache_creation/cache_read, so those three were double-counted once per
        extra block. Recomputed by deduping on `.message.id` — those three taken from
        the group, output taken as MAX. Was $11.08 / 12,764,899 (2.45x over) across
        the same 110 transcript lines = 46 real API calls. See STAGE-053.
    - cycle: ship
      interface: claude-code
      tokens_total: null
      duration_minutes: null
