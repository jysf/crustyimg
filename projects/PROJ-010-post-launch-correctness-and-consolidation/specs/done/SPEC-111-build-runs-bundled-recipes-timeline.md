...
- [x] **ship** — 2026-08-09. PR **#138 merged** as `c91da7b`. Cost totals: **196,572,466 tokens
      / $85.45** across 3 metered cycles of 5 sessions, each reconciled by the orchestrator
      against its own component breakdown at the anchors of the model in `agent` (DEC-083).
      ⚠ 98.0% of that figure is cache re-reads; non-cache-read volume is **3,843,644** — *lower*
      than SPEC-110's 5,352,083 despite a higher headline, because these sessions re-read more
      context rather than doing more work.
      **The design cycle earned its keep against the stage's own framing.** STAGE-039 said this
      needed none ("the fix is wiring, not design"); driving it found the framed fix was
      necessary but **not sufficient**, and that shipping only it would have written the source
      format and silently discarded the modernization — a *quieter* bug than the loud one being
      fixed.
      Verify (Opus) confirmed all 11 ACs by driving against four release binaries, on bytes not
      extensions, with `build` == `apply --recipe X` byte-for-byte on all three recipes and both
      routes — and closed the sweep one level deeper than asked, establishing that
      `registry.build(&step.op, …)` is the only route from an op name to an `Operation`, with one
      production caller. Its punch list was **two over-broad documentation claims and no code
      change**, both fixed, plus three decide-and-record items all resolved.
      **Two orchestrator errors, both recorded rather than quietly fixed.** The "pre-existing
      defect" framing of the cache-collision risk was wrong — verify drove it and showed it is a
      regression *this spec* would have introduced, caught inside the same change; the wrong
      framing had already reached `main` in #139 and was corrected on the branch. And the
      punch-list prompt asked a fix session to correct a file that lives on `main`, producing the
      add/add conflict resolved in `b92feef`; it should have been a separate PR.
      Spec archived to `specs/done/`. **STAGE-039 stays open** — the `docs/data-model.md` chore
      remains, and closing it closes the stage and the last launch-gating repo work.
