...
### FIXED — SPEC-122 built 2026-08-18 (DEC-095, amended)

`Resize::apply` now linearizes, resamples in `F32x4`, and re-encodes. Re-running the **same**
harness against the **branch** binary, with the reference regenerated by the same external tool
(ImageMagick 7.1.2-29 Q16-HDRI):

| case | SSIMULACRA2 before → after | mean signed luma err before → after |
|---|---|---|
| synthetic worst case | **−63.85 → 100.00** | −0.104350 (−88.07%) → 0.000000 (0.00%) |
| `graphic_large.png` | **70.45 → 100.00** | −0.001386 (−0.44%) → 0.000000 (0.00%) |
| `photo_forest_cc0.jpg` | **84.45 → 99.41** | −0.004920 (−2.63%) → 0.000000 (0.00%) |

**The two oracles agree** — perceptual and physical both go to ~zero error, so there is no
disagreement to report. Read the "after" column with the same care DEC-092 asked for the
prototype's: crustyimg and ImageMagick now agree *pixel-exactly* on two of the three cases
(ImageMagick's own `compare -metric AE` returns **0**, and 0.00098 on the photo). That is a real
result against an outside tool — ImageMagick is not the code under test — but the agreement is
tighter than two independent implementations usually manage, and there is a reason: the sRGB OETF
has a slope below 1 above ~0.03 linear, so it *compresses* small differences in the linear
intermediate and more of them round to the same 8-bit code. The photo, the least regular of the
three, is the one that does not land exactly.

Three riders, all measured:

- **The upscale direction was defective in the same way, and the spec did not say so.** AC-6 asked
  for upscale to stay byte-identical to `main`; it cannot, because an upscale *is* a resample.
  Measured against the same reference: `graphic_large.png` 512²→1024² **65.93 → 100.00**,
  `photo_forest_cc0.jpg` 800×532→1600×1064 **89.16 → 98.44**. The no-op (same-size) resize *is*
  byte-identical, on all four colour types.

- **⚠ The op is 3.83× slower, and the transfer function is not why.** `benches/pipeline.rs`
  `resize` (256²→128² RGB, criterion, 100 measurements/arm, Apple M4 Pro): **169.27 µs → 648.52 µs
  (+283.5%, p < 0.05)**. End-to-end the `resize` verb is 1.51–2.45× slower. Decomposed by two
  nested diagnostic builds: the same `F32x4` pipeline with **no** transfer function at all still
  costs 515.88 µs, so **72% of the added time is the `F32x4` working type**, not the maths.
  Replacing the `powf` encode with a threshold table recovers **7 µs of 479** — and is not even
  bit-exact (2 pixels of 160,000 move by 1). A faster path means changing the working type
  (`U16x4` has SIMD kernels `F32x4` does not), which is a design call, not a build one.
